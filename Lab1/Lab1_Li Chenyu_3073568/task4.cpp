...
// Timing Constants (us)
#define PAT_H1      10000U    // 10ms high
#define PAT_L1      20000U    // 20ms low
#define PAT_H2      40000U    // 40ms high
#define PAT_L2      80000U    // 80ms low
#define BLINK_DELAY 500000U   // 500ms LED blink
#define DEBOUNCE    4500U    // 20ms button debounce
...
int main() {
    ...
    while (true) {
        ...
        switch (current_mode) {
            ...
            case MODE_PATTERN:
                led_out = 1;
                // Sequence generator: 10ms high → 20ms low → 40ms high → 80ms low
                switch (pat_step) {
                    case 0:
                        signal_out = 1;
                        if (now - last_signal >= PAT_H1) {
                            last_signal = now;
                            pat_step = 1;
                        }
                        break;
